...
public class TelefonoDistribuidorDAO {
    ...
    public int agregar(TelefonoDistribuidor td) {
        String sql = "insert into TelefonoDistribuidor(numeroPrincipal, numeroSecundario, observaciones, codigoDistribuidor) values (?,?,?,?)";
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            ...
            ps.setInt(4, td.getCodigoDistribuidor());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int Actualizar(TelefonoDistribuidor td) {
        String sql = "update TelefonoDistribuidor set numeroPrincipal = ?, numeroSecundario = ?, observaciones = ?, codigoDistribuidor = ? where codigoTelefonoDistribuidor = ?";
        try{
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            ps.setString(1, td.getNumeroPrincipal());
            ...
            ps.setString(3, td.getObservaciones());
            ps.setInt(4, td.getCodigoDistribuidor());
            ps.setInt(5, td.getCodigoTelefonoDistribuidor());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
